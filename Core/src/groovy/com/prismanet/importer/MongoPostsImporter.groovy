...
//		cursor.addOption(com.mongodb.Bytes.QUERYOPTION_NOTIMEOUT)
	...
	def getUsersByLikes(List<String> userNames){
		DB db = client.getDB("prismanet")
		DBCollection users = db.getCollection("users")
		
		
		DBObject filterIn = new BasicDBObject("\$in",userNames)
		DBObject match = new BasicDBObject("\$match", new BasicDBObject("id", filterIn));
		
		DBObject groupFields = new BasicDBObject( "_id", "\$username");
		groupFields.put("likes", new BasicDBObject( "\$max", "\$likes"));
